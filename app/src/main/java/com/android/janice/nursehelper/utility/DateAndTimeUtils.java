...
public final class DateAndTimeUtils {
    ...
    public static long getTimeWithAddedHrsAndMins(long time, long hours, long mins) {
        return time + hours*HOUR_IN_MILLIS + mins*MINUTE_IN_MILLIS;
    }

    public static long getTimeWithAddedWeeksAndDays(long time, long weeks, long days) {
        return time + weeks*WEEK_IN_MILLIS + days*DAY_IN_MILLIS;
    }

    public static long getTimeWithAddedMonths(long time, long months) {
        return time + months*30*DAY_IN_MILLIS;
    }

    public static long getNearestTimeFromNow(long now, ArrayList<Long> times) {
        if (times == null) {
            Log.e("DATEANDTIMEUTILS","  array of times for 'adminTimes' is NULL!!!");
            return 0;
        }
        int nearest = -1;
        //long now = System.currentTimeMillis();

        Log.i("DATEANDTIMEUTILS"," **************** ");
        Log.i("DATEANDTIMEUTILS","  now: "+String.valueOf(now)+
        "  times[0](9AM): "+times.get(0)+
        "  times[1](10PM) "+times.get(1));
        Log.i("DATEANDTIMEUTILS"," **************** ");

        long diff = now;
        Log.e("DATEANDTIMEUTILS","  diff = "+String.valueOf(diff));
        for (int i = 0; i < times.size(); i++) {
            long time = times.get(i);
            Log.e("DATEANDTIMEUTILS"," time = "+String.valueOf(time) + "   now = "+String.valueOf(now));
            if (time > now) {
                Log.e("DATEANDTIMEUTILS"," time > now ...");
                if ((time - now) < diff) {
                    diff = Math.abs(now - time);
                    nearest = i;
                    Log.e("DATEANDTIMEUTILS"," nearest = "+String.valueOf(nearest));
                }
            }
        }
        if (nearest < 0) {
            return 0;  // COULD be ok - if near midnight, just wait for next day ....
        }
        return times.get(nearest);
    }

    public static long getTimestampForHoursAndMins(long hours, long minutes) {
        // First get current date/time
        long todaysDate = System.currentTimeMillis();
        // Now, normalize the date to be today, at hour 0:00:00 (midnight)
        //long todayAtMidnight = normalizeDate(todaysDate);
        long todayAtMidnight = getNormalizedUtcDateForToday();
        // Now, add the hour (in millisec) to 'todayAtMidnight' to get the date/time of this 'hour',
        //  for today:
        long timestamp = todayAtMidnight + (hours)*HOUR_IN_MILLIS +
                (minutes)*MINUTE_IN_MILLIS;
        Log.i("DATEANDTIMEUTILS"," today:"+String.valueOf(todaysDate)+
                "  todayAtMidnight: "+String.valueOf(todayAtMidnight)+
                "  timestamp (added hours/mins) "+String.valueOf(timestamp));
        Log.i("DATEANDTIMEUTILS"," today:"+getFormattedDateFromLongTEMP(todaysDate)+
                "  todayAtMidnight: "+getFormattedDateFromLongTEMP(todayAtMidnight)+
                "  timestamp (added hours/mins) "+getFormattedDateFromLongTEMP(timestamp));
        return timestamp;
    }

    private static String getFormattedDateFromLongTEMP(long dateTime) {
        Date date = new Date(dateTime);
        String formattedDateTime = new SimpleDateFormat("MM-dd-YY HH:mm").format(date);
        return formattedDateTime;
    }

    public static String getFormattedDateFromLong(Context context, long dateTime) {
        Date date = new Date(dateTime);
        String formatDateTime = context.getString(R.string.format_admin_date_time);
        String formattedDateTime = new SimpleDateFormat(formatDateTime).format(date);
        return formattedDateTime;
    }

    public static long getLongFromFormattedDate(Context context, String formattedDate) {
        DateFormat formatter = new SimpleDateFormat(context.getString(R.string.format_admin_date_time));
        long time = 0;
        try {
            Date date = (Date) formatter.parse(formattedDate);
            time = date.getTime();
        } catch (ParseException e){
            Log.e("UTILS","  Problem parsing date/time string: '"+formattedDate+"'  using formatter: '"+
                    (context.getString(R.string.format_admin_date_time))+"'");
        }
        return time;
    }

    public static boolean formattedTimesWithinOneHour() {

        //String str_date="13-09-2011";
        //DateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
        //String formattedDate = new SimpleDateFormat("MM-dd-YY HH:mm:ss").format(date);
        String str_date="09-13-2011 09:05";
        String str_date2="09-13-2011 10:06";
        // later, replace below w/ new SimpleDateFormat(<format-string>, Locale locale);
        //   where locale could be Locale.US
        DateFormat formatter = new SimpleDateFormat("MM-dd-yyyy HH:mm");

        try {
            Date date1 = (Date) formatter.parse(str_date);
            Date date2 = (Date) formatter.parse(str_date2);
            //Log.i("UTILS","Today is " + date1.getTime());
            long time1 = date1.getTime();
            long time2 = date2.getTime();
            boolean within = timesWithinOneHour(time1, time2);
            Log.i("UTILS",str_date+"  and  "+str_date2+
                " is "+(within ? "within ":"NOT within ")+"one hour of each other");
            return timesWithinOneHour(time1, time2);
        } catch (ParseException e) {
            Log.i("UTILS"," !!!! PROBLEM ... PARSE EXCEPTION");
        }
        return false;
    }

    public static boolean timesWithinOneHour(long time1, long time2) {

        long timeDiff = Math.abs(time2-time1);
        // (since times are in millis, convert to hours first:
        Log.i("UTILS","   time2 - time1 "+String.valueOf(timeDiff));
    ...
    private static int getDayDifference(Context context, long dateInMillis) {
        long daysFromEpochToProvidedDate = elapsedDaysSinceEpoch(dateInMillis);
        long daysFromEpochToToday = elapsedDaysSinceEpoch(System.currentTimeMillis());
